...
class HS_DY(Optimizer):
    ...
    def __init__(
        ...
        super(HS_DY, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    state['beta'] = max(0, 
                    min(torch.dot(d_p.data.reshape(-1), (d_p.data.reshape(-1) - state['g'].reshape(-1))) \
                ...
                if line_search == 'None':
                    if state['index']:
                        state['A'] = HS_DY._get_A(p, d_p)
                        state['alpha'] = HS_DY.Exact(state['A'], d_p, state['d'])
                    else:
                        state['alpha'] = HS_DY.Exact(state['A'], d_p, state['d'])
