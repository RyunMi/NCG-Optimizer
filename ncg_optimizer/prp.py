...
class PRP(Optimizer):
    r"""Implements Polak Ribiere Polyak Conjugate Gradient.
    ...
    def __init__(
        ...
        super(PRP, self).__init__(params, defaults)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if line_search == 'None':
                    if state['index']:
                        state['A'] = PRP._get_A(p, d_p)
                        state['alpha'] = PRP.Exact(state['A'], d_p, state['d'])
                    else:
                        state['alpha'] = PRP.Exact(state['A'], d_p, state['d'])
